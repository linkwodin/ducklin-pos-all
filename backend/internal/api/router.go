...
func SetupRouter(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	protected.Use(authMiddleware(cfg.JWTSecret))
	{
		...
		protected.GET("/stock/wholesale-ship-from", stockHandler.GetWholesaleShipFromMap)
		...
		// Wholesale order stats (reporting)
		protected.GET("/wholesale-orders/stats/revenue-summary", wholesaleOrderHandler.GetWholesaleRevenueSummaryStats)
		protected.GET("/wholesale-orders/stats/product-sales", wholesaleOrderHandler.GetWholesaleProductSalesStats)
		protected.GET("/wholesale-orders/stats/client-sales", wholesaleOrderHandler.GetWholesaleClientSalesStats)
		protected.POST("/wholesale-orders/test-email", wholesaleOrderHandler.SendTestEmail)
		protected.POST("/wholesale-orders/bulk-attachments-zip-email", wholesaleOrderHandler.BulkAttachmentsZipEmail)
		...
		protected.GET("/wholesale-orders/:id/endorse-allocation-preview", wholesaleOrderHandler.EndorseAllocationPreview)
		protected.PUT("/wholesale-orders/:id/reject", wholesaleOrderHandler.Reject)
		protected.PUT("/wholesale-orders/:id/archive", wholesaleOrderHandler.Archive)
		protected.PUT("/wholesale-orders/:id/resubmit", wholesaleOrderHandler.Resubmit)
		protected.PUT("/wholesale-orders/:id/assign", wholesaleOrderHandler.AssignStores)
		protected.PUT("/wholesale-orders/:id/unassign", wholesaleOrderHandler.UnassignStores)
		protected.PUT("/wholesale-orders/:id/assign-by-defaults", wholesaleOrderHandler.AssignByDefaults)
		...
		protected.PATCH("/wholesale-orders/:id/invoice-sent", wholesaleOrderHandler.SetInvoiceSentAt)
		protected.GET("/wholesale-orders/:id/audit-logs", wholesaleOrderHandler.GetAuditLogs)
		protected.POST("/wholesale-orders/:id/audit-logs/:auditLogId/restore-document", wholesaleOrderHandler.RestoreDocumentFromAudit)
		protected.POST("/wholesale-orders/:id/email", wholesaleOrderHandler.EmailOrder)
		protected.POST("/wholesale-orders/:id/skip-email", wholesaleOrderHandler.SkipWholesaleOrderEmail)
		protected.POST("/wholesale-orders/:id/email-document", wholesaleOrderHandler.EmailDocument)
		protected.POST("/wholesale-orders/:id/po-attachments", wholesaleOrderHandler.UploadPOAttachments)
		protected.DELETE("/wholesale-orders/:id/documents/:docId", wholesaleOrderHandler.DeletePOAttachment)
		protected.GET("/wholesale-orders/:id/documents/:docId/download", wholesaleOrderHandler.DownloadDocument)
		protected.GET("/wholesale-orders/:id/legacy-payment-proof/download", wholesaleOrderHandler.DownloadLegacyPaymentProof)
		protected.POST("/wholesale-orders/:id/upload-payment-proof", wholesaleOrderHandler.UploadPaymentProof)
		protected.POST("/wholesale-orders/:id/confirm-payment", wholesaleOrderHandler.ConfirmPayment)
		...
		protected.PATCH("/shipments/:id/status", wholesaleOrderHandler.UpdateShipmentStatus)
		protected.POST("/shipments/:id/start-shipment", wholesaleOrderHandler.StartShipment)
		protected.POST("/shipments/:id/upload-signed-delivery-note", wholesaleOrderHandler.UploadSignedDeliveryNote)
